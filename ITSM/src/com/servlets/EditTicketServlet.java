...
public class EditTicketServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			String sql = "UPDATE ticket status = ? where ticket_id = ?";
		...
		String sql = "UPDATE ticket_detail SET Projectcode = ?, ActivityReport = ?, Date = ?, Customer = ?, Location = ?, Contact_Person = ?, Tel = ?, Solution = ?, Status = ?, Configuration = ?, code = ? where ticket_id_fk = ?";
		try {
			PreparedStatement ps = conn.prepareStatement(sql);
