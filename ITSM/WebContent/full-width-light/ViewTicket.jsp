														...
														<tr>
															<td><c:set var="role" value="${row.sla}" /> <c:choose>
																	<c:when test="${role == 'Critical'}">
																		<span class="label label-danger" style="background-color: #DB1E1E;">
																	</c:when>
																	<c:when test="${role == 'Major'}">
																		<span class="label label-danger" style="background-color: #FF4200;">
																	</c:when>
																	<c:when test="${role == 'Minor'}">
																		<span class="label" style="background-color: #FFA500;">
																	</c:when>
																	<c:otherwise>
																		<span class="label label-warning" style="background-color: #F3B782;">
																	</c:otherwise>
												...
												<div class="row">
													...
													<div class="col-lg-12">
														<div class="form-group">
															...
															<p class="form-control-static" id="v9">${row.incident}</p>
														</div>
													</div>
													<div class="col-lg-12">
														<div class="form-group">
		...
		var x = setInterval(function() {
			for (var i = 1, row; row = table.rows[i]; i++) {
				...
				if (status.innerHTML.includes('pending')) {
					...
					if(days <= 0 && hours <= 0 && minutes == 10 && seconds >= 50 && countDownDate > now){
					...
					else if(days <= 0 && hours <= 0 && minutes == 0 && seconds <= 10 && countDownDate > now){
						var snd = new Audio('dist/sound/beep-06.mp3');
						snd.play();
						$(".tst2").change(function () {
							$.toast().reset('all');
							$("body").removeAttr('class');
							$.toast({
					            heading: 'Warning! EARNO '+ ear + " Over SLA in 10 seconds",
					            text: 'Please hurry to fix the case.',
					            position: 'top-right',
					            loaderBg:'#fec107',
					            icon: 'warning',
