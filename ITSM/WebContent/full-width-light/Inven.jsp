...
<body>
	...
	<sql:query dataSource="${ticket}" var="result">
     	SELECT hostname, ip, name, descr, pid, serial from switch_inven;
     </sql:query>
			...
			<div class="container-fluid">
				...
				<div class="row heading-bg">
					<div class="col-lg-3 col-md-4 col-sm-4 col-xs-12">
						<h5 class="txt-dark">Inventory Store</h5>
					</div>
				</div>
				<!-- /Title -->
												...
												<tbody>
													<c:forEach var="row" items="${result.rows}">
														...
														<tr>
															...
															<c:choose>
																<c:when test="${pcode.rowCount == 0}">
																	<td>N/A</td>
																</c:when>
																<c:otherwise>
																	<c:forEach items="${pcode.rows}" var="row1">
																		<c:set var="checkpcode" value="${row1.p_code}" />
																		<c:if test="${checkpcode == '310171627' || checkpcode == 'SDA'}">
																			<td>SDA</td>
																		</c:if>
																		<c:if test="${checkpcode == '501180681' || checkpcode == 'Voice'}">
																			<td>VOICE</td>
																		</c:if>
																		<c:if test="${checkpcode == '501180872' || checkpcode == 'FIDS'}">
																			<td>FIDS</td>
																		</c:if>
																	</c:forEach>
																</c:otherwise>
